...
def test_model_convergence(jump, mle, X, sample_lengths=(250, 500, 1000, 2000)):
    ...
    for sample_length in tqdm.tqdm(sample_lengths):
        ...
        for seq in tqdm.tqdm(range(X.shape[1])):
            ...
            data['jump']['tpm_11'].append(jump.tpm[0, 0])
            if len(jump.tpm) > 1:
                data['jump']['tpm_22'].append(jump.tpm[1, 1])
            else:
                data['jump']['tpm_22'].append(0)

            data['mle']['mu_1'].append(mle.mu[0])
            data['mle']['mu_2'].append(mle.mu[1])
            data['mle']['std_1'].append(mle.std[0])
            data['mle']['std_2'].append(mle.std[1])
            data['mle']['tpm_11'].append(mle.tpm[0, 0])
            if len(mle.tpm) > 1:
                data['mle']['tpm_22'].append(mle.tpm[1, 1])
            else:
                data['mle']['tpm_22'].append(0)
    ...
    for sample_length in sample_lengths:
        true_data = \
            {'model': 'true',
             ...
             'std_1': sampler.std[0],
             'std_2': sampler.std[1],
             'tpm_11': sampler.tpm[0, 0],
             'tpm_22': sampler.tpm[1, 1],
