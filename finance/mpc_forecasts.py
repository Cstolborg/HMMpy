...
values of 0 and covariances of 0. This is a crude approximation. Has to handle these cases better.
        ...
        for t in tqdm.trange(window_len, len(df)):
            ...
            if debug == True:
                if np.any(np.isnan(preds)) == True:
                    print('t: ', t)
                    print(preds)
                    print('NaNs in cov: ', np.any(np.isnan(self.cov[t - window_len])))
